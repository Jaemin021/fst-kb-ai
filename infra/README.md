...
- Nginx에는 `proxy_set_header Host $host;`(결과 이미지 URL 생성에
  ...
  설정되어 있습니다.
- 외부에서 접근 가능한 포트는 22(SSH)와 80(HTTP)입니다. 8000번으로의
  외부 접근이 차단되어 있는 것을 확인했습니다(2026-08-26, 연결 시간
  초과). 다만 uvicorn 자체는 `0.0.0.0:8000`에 바인딩되어 있어 차단을
  담당하는 것은 보안 그룹입니다. **8000 인바운드 규칙을 열면 Nginx의
  헤더 주입·용량 제한·타임아웃을 우회해 백엔드가 그대로 노출되므로
  열지 마십시오.** 근본적으로는 systemd 유닛의 `ExecStart`를
