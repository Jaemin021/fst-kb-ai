...
**Response 200** (값은 형식을 보이기 위한 예시이며 실제 측정값이
아닙니다 — 현재 가중치에서 `multiRiding`은 항상 0입니다)
...
| 413 | 파일이 10MB를 초과 (배포 환경에서 12MB를 넘으면 Nginx가 먼저 차단하며, 이때 응답 본문은 JSON이 아닌 HTML입니다) |
