...
- YOLO 모델이 분석한 실제 결과 표시
  ...
  - 안전모 미착용·다인 탑승 탐지 시 경고 문구
    (다인 탑승은 화면·집계까지 구현되어 있으나 현재 가중치가 해당
    클래스를 탐지하지 못합니다 — "현재 제한 사항" 참고)
  - 바운딩 박스가 그려진 결과 이미지
...
- 모델 정확도가 아직 낮은 편입니다. 악천후, 인물 겹침 등 어려운
  조건에서 미탐지가 발생해 학습 데이터 보강을 진행할 예정입니다.
...
- 결과 이미지가 서버 디스크에 계속 쌓이며, `/results/<파일명>`으로
  인증 없이 공개됩니다. 주기적 정리 정책은 아직 없습니다. 사람이
  찍힌 사진을 다루므로 시연 종료 시 인스턴스와 함께 삭제할 계획입니다.
- 업로드 기록 저장, 로그인 기능은 없습니다.
- 부하·동시 접속 테스트는 하지 않았습니다. uvicorn 워커 1개에 CPU
  추론이라 요청이 순차 처리됩니다.
...
7. ~~YOLO 입력 채널 순서 버그 수정, 추론 디버그 스크립트 추가~~
   (2026-08-26 배포 완료)
8. 모델 정확도 개선 — 특히 `multi_riding` 학습 데이터 보강 (진행 중)
9. HTTPS 적용, 결과 이미지 정리 정책 (선택 과제)
